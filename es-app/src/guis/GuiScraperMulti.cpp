	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "正在进行搜刮", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	mGrid.setEntry(mTitle, Vector2i(0, 0), false, true);

	mSystem = std::make_shared<TextComponent>(mWindow, "游戏系统", Font::get(FONT_SIZE_MEDIUM), 0x777777FF, ALIGN_CENTER);
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "游戏 " << (mCurrentGame + 1) << " 个 " << mTotalGames << " - " << strToUpper(mSearchQueue.front().game->getPath().filename().string());
...
void GuiScraperMulti::finish()
{
	...
	if(mTotalSuccessful == 0)
	{
		...
		ss << mTotalSuccessful << " 游戏" << ((mTotalSuccessful > 1) ? "" : "") << " 搜刮成功!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " 游戏" << ((mTotalSkipped > 1) ? "" : "") << " SKIPPED.";
	}
